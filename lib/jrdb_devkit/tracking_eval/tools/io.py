...
def load_motchallenge(fname, **kwargs):
    ...
    if not _3d:
        df = pd.read_csv(
            ...
            header=None,
            names=['FrameId', 'Id', 'X', 'Y', 'Width', 'Height', 'X0', 'Y0', 'Z0', 'Length0', 'Height0', 'Width0', 'Theta0', 'Confidence'],
            engine='python'
        ) # , 'ClassId', 'Visibility'],

        # Account for matlab convention.
        # Removed from original script. We use python.
        # df[['X', 'Y']] -= (1, 1)
    else:
        df = pd.read_csv(
            fname,
            ...
            skipinitialspace=True,
            header=None,
            names=['FrameId', 'Id', 'X0', 'Y0', 'Width0', 'Height0', 'X', 'Y', 'Z', 'Length', 'Height', 'Width', 'Theta', 'Confidence'],
            engine='python'
        ) # , 'ClassId', 'Visibility'],
        # Account for matlab convention.
        # Removed from original script. We use python.
