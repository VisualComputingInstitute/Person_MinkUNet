...
try:
    ...
    from evaluation.mot import MOTAccumulator
    from evaluation.lap_util import linear_sum_assignment
...
class MetricsHost:
    ...
    def compute_many(self, dfs, metrics=None, names=None, generate_overall=False,
                     sequence_splits=None, _3d=False, dist_cutoffs=[5, 10, 15, 20]):
        ...
        # if _3d:
        overall_df_idx = len(dfs) - 1
        dist_cutoffs.insert(0, 0)
        for idx in range(len(dist_cutoffs)):
            if idx == len(dist_cutoffs) - 1:
                name = "{}+m".format(dist_cutoffs[idx])
                temp_df = dfs[overall_df_idx][dfs[overall_df_idx]['dist'] >= dist_cutoffs[idx]]
            else:
                name = "{}-{}m".format(dist_cutoffs[idx], dist_cutoffs[idx + 1])
                temp_df = \
                    dfs[overall_df_idx][(dfs[overall_df_idx]['dist'] >= dist_cutoffs[idx])
                                        & (dfs[overall_df_idx]['dist'] < dist_cutoffs[idx + 1])]
            names.append(name)
            dfs += [temp_df]

        # Compute thresholded overall at 15m
        if _3d:
            names.append('OVERALL (15m)')
            temp_df = dfs[overall_df_idx][dfs[overall_df_idx]['dist'] <= 15]
            dfs += [temp_df]

        if sequence_splits is not None:
            for name, split in sequence_splits.items():
                split_sequences = []
                for seq in split:
                    if not _3d:
                        seq += '_image_stitched'
                    try:
                        ...
                        print("ValueError:", seq)
                        continue
                    split_sequences.append(dfs[idx])
                if split_sequences:
                    dfs += [MOTAccumulator.merge_event_dataframes(split_sequences)]
                    names += [name]
            ...
            if _3d:
                ...
                for name, split in sequence_splits.items():
                    ...
                    for seq in split:
                        ...
                        split_sequences.append(dfs[idx])
                    if split_sequences:
                        temp_df = MOTAccumulator.merge_event_dataframes(split_sequences)
                        temp_df = temp_df[temp_df['dist'] <= 15]
                        dfs += [temp_df]
                        names += [name+' (15m)']
...
def motp(df, num_detections):
    ...
    return df.noraw['D'].sum() / (num_detections + 0.00000001) # epsilon added to account for 0 detections case
