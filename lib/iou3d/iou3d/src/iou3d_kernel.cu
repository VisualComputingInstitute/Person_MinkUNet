...
__device__ inline float dist_bev(const float *box_a, const float *box_b, const float l_ave, const float w_ave){
    // params: box_a (5) [x1, y1, x2, y2, angle]
    // params: box_b (5) [x1, y1, x2, y2, angle]
    float cx_a = 0.5 * (box_a[0] + box_a[2]);
    float cy_a = 0.5 * (box_a[1] + box_a[3]);

    float cx_b = 0.5 * (box_b[0] + box_b[2]);
    float cy_b = 0.5 * (box_b[1] + box_b[3]);

    float dist = sqrt((cx_b - cx_a) * (cx_b - cx_a) + (cy_b - cy_a) * (cy_b - cy_a));

    float ct = atan2(cy_b - cy_a, cx_b - cx_a);
    float ct_a = abs(cos(box_a[4] - ct));
    float ct_b = abs(cos(box_b[4] - ct));

    float dist_norm = w_ave + 0.5 * (l_ave - w_ave) * (ct_a + ct_b);
...
__global__ void nms_dist_kernel(const int boxes_num, const float l_ave, 
                                const float w_ave, const float nms_overlap_thresh,
                                const float *boxes, unsigned long long *mask){
    //params: boxes (N, 5) [x1, y1, x2, y2, ry]
    //params: mask (N, N/THREADS_PER_BLOCK_NMS)
    ...
    if (threadIdx.x < row_size) {
        ...
        for (i = start; i < col_size; i++) {
            if (dist_bev(cur_box, block_boxes + i * 5, l_ave, w_ave) < nms_overlap_thresh){
                t |= 1ULL << i;
            }
        }
        const int col_blocks = DIVUP(boxes_num, THREADS_PER_BLOCK_NMS);
...
void nmsDistLauncher(const float *boxes, unsigned long long * mask, int boxes_num, float l_ave, float w_ave, float nms_overlap_thresh){
    dim3 blocks(DIVUP(boxes_num, THREADS_PER_BLOCK_NMS),
                DIVUP(boxes_num, THREADS_PER_BLOCK_NMS));
    dim3 threads(THREADS_PER_BLOCK_NMS);
    nms_dist_kernel<<<blocks, threads>>>(boxes_num, l_ave, w_ave, nms_overlap_thresh, boxes, mask);
}
