...
int nms_dist_gpu(at::Tensor boxes, at::Tensor keep, float l_ave, float w_ave, float nms_overlap_thresh){
    // params boxes: (N, 5) [x1, y1, x2, y2, ry]
    // params keep: (N)
    ...
    unsigned long long *mask_data = NULL;
    CHECK_ERROR(cudaMalloc((void**)&mask_data, boxes_num * col_blocks * sizeof(unsigned long long)));
    nmsDistLauncher(boxes_data, mask_data, boxes_num, l_ave, w_ave, nms_overlap_thresh);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("nms_dist_gpu", &nms_dist_gpu, "oriented distance based nms gpu");
}
